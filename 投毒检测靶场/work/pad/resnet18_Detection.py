...
class ResNet18Detector:
    ...
    def filter_poisoned_samples(losses_history, all_filenames, threshold_ratio=0.1):
        ...
        # 计算每个样本的平均损失
        avg_losses = {filename: sum(losses) / len(losses) for filename, losses in losses_history.items()}
        
        # 计算每个样本的损失变化率
        loss_changes = {}
        for filename, losses in losses_history.items():
            if len(losses) > 1:
                loss_change = losses[-1] - losses[0]
                loss_changes[filename] = loss_change
        
        # 对损失变化率进行排序
        sorted_loss_changes = sorted(loss_changes.items(), key=lambda item: item[1], reverse=True)
        
        # 计算阈值
        threshold_index = int(len(sorted_loss_changes) * threshold_ratio)
        if threshold_index == 0:
            threshold_index = 1  # 确保至少有一个样本被检测到
        
        # 筛选对抗样本
        poisoned_filenames = set([filename for filename, _ in sorted_loss_changes[:threshold_index]])
