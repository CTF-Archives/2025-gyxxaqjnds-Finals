...
def process_video(face_detector, video_p, crop_dir, args, ids=0):
        ...
        if len(batch_frames) == BATCH_SIZE or (idx == frames_num - 1
            ...
            for i in range(n_frame):
                ...
                for j in range(n_face):
                    ...
                    if True: # is_fake_by_mask(batch_mask_frames[i], square_box):
                        # save cropping
                        h, w, _ = frame.shape
                        # ensure square
                        x_w = xmax - xmin
                        ...
                        crop_p = osp.join(crop_dir,
                                        ids+CROP_NAME_PATTERN.format(frame_idx))
...
def main(args):
    ...
    video_ps=glob.glob(os.path.join(args.src_root, "*"))
    ...
    for video in tqdm(video_ps):
        ...
        ids = os.path.split(video)[-1].split(".mp4")[0]
        save_dir = args.dst_root

        os.makedirs(save_dir, exist_ok=True)


        process_video(face_detector, video, save_dir, args, ids=ids)
...
if __name__ == '__main__':
    ...
    arg('--gpus', type=str, default='3')
    arg('--fake-type', type=str, default='FaceSwap',choices=["FaceShifter", "Deepfakes", "Face2Face", "FaceSwap", "NeuralTextures", "DeepFakeDetection", "real"])
    arg('--src-root', type=str, default='/data/sxt_data/FaceForensics++/')
    arg('--dst-root', type=str, default='/data/sxt_data/FaceForensics++/')
